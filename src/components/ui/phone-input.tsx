    ...
    <Popover>
      <PopoverTrigger asChild>
        <Button
          ...
          variant={"outline"}
          className={cn(
            "flex gap-1.5 rounded-e-none rounded-s-md px-3 h-11 border-r border-input/50 bg-muted/50 hover:bg-muted transition-colors focus:ring-0"
          )}
          disabled={disabled}
        >
      ...
      <PopoverContent 
        className="w-[300px] p-0 z-[100] bg-card border border-border shadow-2xl rounded-xl" 
        align="start"
      >
        <Command className="bg-transparent">
          ...
          <CommandList className="max-h-72 overflow-y-auto w-full">
            <CommandEmpty className="py-6 text-center text-sm text-muted-foreground font-medium">Không tìm thấy quốc gia.</CommandEmpty>
            <CommandGroup heading="Tất cả quốc gia" className="px-2">
              {options
                .filter((x) => x.value)
                .map((option) => (
                  <CommandItem
                    className="gap-3 cursor-pointer py-2.5 px-3 rounded-lg hover:bg-muted transition-all text-foreground aria-selected:bg-muted font-medium opacity-100"
                    key={option.value}
                  ...
                  >
                    ...
                    />
                    <span className="flex-1 text-sm font-semibold text-foreground whitespace-nowrap overflow-hidden text-ellipsis">
                      {option.label}
                    </span>
                    {option.value && (
...
const FlagComponent = ({ country, countryName }: FlagProps) => {
  ...
  return (
    <span className="flex h-5 w-5 shrink-0 items-center justify-center overflow-hidden rounded-full border border-border bg-muted shadow-sm transition-all group-hover:border-primary/30">
      {Flag && (
