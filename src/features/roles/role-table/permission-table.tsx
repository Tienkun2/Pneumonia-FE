...
export function PermissionTable({ table, columns, globalFilter, isLoading, hidePagination = false }: PermissionTableProps) {
  ...
  return (
    <div className="rounded-[24px] bg-card/60 backdrop-blur-xl shadow-xl shadow-primary/5 flex flex-col overflow-hidden border border-border/40 relative">
      ...
      <div className={cn("overflow-x-auto overflow-y-auto transition-all duration-300", isLoading && hasRows ? "opacity-50 pointer-events-none blur-[0.5px]" : "opacity-100")} style={{ maxHeight: "calc(100vh - 420px)", minHeight: 400 }}>
                ...
                {headerGroup.headers.map((header) => (
                  ...
                  >
                    {header.isPlaceholder ? null : (
                      <div className={`flex items-center ${header.id === "actions" ? "justify-end pr-4" : (header.id === "STT" || ["view", "create", "update", "delete", "status", "assign"].includes(header.id)) ? "justify-center" : "justify-start"}`}>
